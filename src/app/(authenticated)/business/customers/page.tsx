...
import { Calculator, Gift, DollarSign, CheckCircle, AlertCircle, UserPlus, User, Crown, Users, History, Search, X } from 'lucide-react';
...
export default function BusinessCashierPage() {
  ...
  const [inProgressMissions, setInProgressMissions] = useState<MissionRegistry[]>([]);
  const [loadingMissions, setLoadingMissions] = useState(false);
  const [showMissionModal, setShowMissionModal] = useState(false);
  const [selectedMission, setSelectedMission] = useState<MissionRegistry | null>(null);
  ...
  const fetchInProgressMissions = async () => {
    ...
    try {
      const res = await fetch(`/api/business/mission-registry?status=in_progress&customer_id=${customer.id}`);
  ...
  const completeMission = async (registryId: number, status: 'completed' | 'failed') => {
    try {
      ...
      toast.success(`Mission ${status === 'completed' ? 'completed' : 'failed'} successfully`);
  ...
  const selectCustomerFromList = async (selectedCustomer: CustomerListItem) => {
    ...
    const phoneNumber = parsePhoneNumberFromString(selectedCustomer.phone_number, 'IN');
    if (phoneNumber?.isValid()) {
      const formattedPhone = phoneNumber.format('E.164');
      setLoadingCustomer(true);

      try {
        const res = await fetch(`/api/business/customers/search?phone=${encodeURIComponent(formattedPhone)}`);
        if (res.ok) {
  ...
  const filteredCustomers = customerList.filter(customer =>
    (customer.name?.toLowerCase().includes(customerSearchTerm.toLowerCase()) ?? false) ||
  ...
  const calculateFinalAmount = (): number => {
    ...
    if (manualDiscountPercentage) {
      const percentageDiscount = (bill * (parseFloat(manualDiscountPercentage) || 0)) / 100;
  ...
  const discountAmount = customer && billAmount ? parseFloat(billAmount) - finalAmount : 0;
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="mb-8">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Cashier Portal</h1>
              <p className="text-gray-600">Process customer transactions and redeem loyalty rewards</p>
            </div>
            <Button
          ...
          <div className="space-y-6">
            ...
            <Card className="border-2 border-blue-300 bg-blue-50 shadow-md">
              ...
              <CardContent className="space-y-4 p-6">
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <Button
                      ...
                      className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold px-6 py-2 shadow-lg hover:shadow-xl transition-all duration-300"
                    >
                ...
                {customer && (
                  <div className={`border rounded-lg p-4 shadow-sm ${isNewCustomer ? 'bg-blue-50 border-blue-200' : 'bg-green-50 border-green-200'}`}>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className={`font-semibold text-base ${isNewCustomer ? 'text-blue-800' : 'text-green-800'}`}>
                        {isNewCustomer ? 'New Customer Enrolled' : 'Customer Found'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">
                          Tier: <span className="font-medium">{customer.current_tier_name}</span>
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">
                          Points Rate: <span className="font-medium">{customer.points_rate || 1} point{(customer.points_rate || 1) === 1 ? '' : 's'} per ₹1</span>
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">
                          Next Tier: <span className="font-medium">
                            {customer.current_tier.points_to_unlock > customer.points
                ...
                {billAmount && customer && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-blue-900">₹{parseFloat(billAmount).toFixed(2)}</span>
                    </div>
                    {discountAmount > 0 && (
                    ...
                    <div className="flex justify-between items-center mt-2 pt-2 border-t border-blue-200">
                      ...
                      <span className="text-xl font-bold text-blue-900">₹{finalAmount.toFixed(2)}</span>
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-blue-200">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-blue-900">
                          {Math.floor(parseFloat(billAmount) * (customer.points_rate || 1))}
                        </span>
                      </div>
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <>
                        ...
                        Process Transaction - Final Amount: ₹{finalAmount.toFixed(2)}
                      </>
                    )}
                  </Button>
                )}
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="text-xs text-gray-500 space-y-1">
                                <p>Customer: {mission.customer_name} ({mission.customer_phone})</p>
                                <p>Started: {new Date(mission.started_at).toLocaleDateString()}</p>
                              </div>
                            </div>
                            <Button
                              size="sm"
              ...
              <CardContent>
                {!customer ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p className="text-sm text-gray-400 mt-1">Customer can still earn points!</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {customer.current_tier.rewards.map((reward) => {
                      ...
                      return (
                        <div
                          key={reward.id}
                          className={`border rounded-lg p-4 transition-all ${isRedeemed
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <h4 className="font-medium text-gray-900">
                                {reward.reward_type === 'cashback' && `${reward.percentage}% Cashback`}
                                {reward.reward_type === 'limited_usage' && reward.reward_text}
                                {reward.reward_type === 'custom' && `${reward.name}: ${reward.reward}`}
                              </h4>
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-4 text-sm text-gray-500 ml-13">
                              <div className="flex items-center gap-1">
                                ...
                                <span>{customer.current_tier_name ?? 'No Tier'}</span>
                              </div>
                              <div className="flex items-center gap-1">
        ...
        {showMissionModal && selectedMission && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
              <div className="p-6">
                <h3 className="text-lg font-semibold mb-4">Complete Mission</h3>

                <div className="mb-4">
                  <p className="text-sm text-gray-600 mb-2">Mission: {selectedMission.mission_title}</p>
                  <p className="text-sm text-gray-600 mb-2">Customer: {selectedMission.customer_name}</p>

                  {/* Prominent Offer Display */}
                  <div className="mt-3 p-3 border-2 border-dotted border-blue-300 bg-blue-50 rounded-lg">
                    <p className="text-sm text-blue-800 font-medium mb-1">Mission Offer:</p>
                    <p className="text-lg font-bold text-blue-900">{selectedMission.mission_offer}</p>
                  </div>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  <Button
                    onClick={() => completeMission(selectedMission.id, 'completed')}
                    className="flex-1 bg-green-600 hover:bg-green-700"
                  >
                    Complete Mission
                  </Button>
                  <Button
                    onClick={() => completeMission(selectedMission.id, 'failed')}
                    variant="outline"
                    className="flex-1"
                  >
                    Mark Failed
