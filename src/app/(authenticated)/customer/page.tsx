      ...
      <div className="container mx-auto py-8 px-4">
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Tier Progress for Registered Shops */}
            <div>
              ...
              {shops.length > 0 ? (
                <div className="space-y-3">
                  {shops.slice(0, 3).map((shop) => {
                    ...
                    const currentTierLevel = tierOrder[shop.currentTier as keyof typeof tierOrder] || 1;
                    const nextTierLevel = Math.min(currentTierLevel + 1, 5);
                    const nextTierName = ['Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond'][nextTierLevel - 1];
                    const progressPercentage = Math.min((shop.loyaltyPoints / (nextTierLevel * 100)) * 100, 100);

                    return (
                      <Card key={shop.shopId} className="border border-gray-200 hover:border-blue-300 transition-all duration-200">
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <h3 className="font-semibold text-sm text-gray-800">{shop.shopName}</h3>
                                <p className="text-xs text-gray-500">{shop.currentTier} Tier</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="text-sm font-bold text-blue-600">{shop.loyaltyPoints}</div>
                ...
                <Card className="border-2 border-dashed border-gray-300 bg-gray-50">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-sm text-gray-500">No shops registered yet. Start shopping to earn points!</p>
                  </CardContent>
                </Card>
              )}
            </div>
