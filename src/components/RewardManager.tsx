...
export function RewardManager({ rewards, onRewardsChange, disabled = false }: RewardManagerProps) {
    ...
    const updateReward = () => {
        if (!editingReward || editingIndex === -1 || !validateReward(rewardInput)) return;
        ...
        <div className="space-y-6">
            ...
            {rewards.length > 0 && (
                <div className="space-y-3">
                    <h4 className="text-lg font-semibold text-gray-800">Current Rewards</h4>
                    <div className="grid gap-3">
                        {rewards.map((reward, index) => (
                            <Card key={reward.id ?? index} className="border border-gray-200 shadow-sm">
                ...
                <CardContent className="space-y-4">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            onChange={(e) => setRewardInput({ ...rewardInput, reward_type: e.target.value as Reward['reward_type'] })}
                            disabled={disabled}
                    ...
                    {rewardInput.reward_type === 'cashback' && (
                        <div>
                            <Label className="text-sm font-medium text-gray-700">Cashback Percentage</Label>
                            <Input
                                type="number"
                                min="1"
                                max="100"
                                value={rewardInput.percentage ?? ''}
                                onChange={(e) => setRewardInput({ ...rewardInput, percentage: Number(e.target.value) })}
                                disabled={disabled}
                                placeholder="e.g., 5"
                                className="mt-1 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            />
                            <p className="mt-1 text-sm text-gray-500">Enter a percentage between 1-100</p>
                        </div>
                    )}
                    ...
                    {rewardInput.reward_type === 'limited_usage' && (
                        <div className="space-y-4">
                            <div>
                                <Label className="text-sm font-medium text-gray-700">Reward Text</Label>
                                <Input
                                    value={rewardInput.reward_text ?? ''}
                                    onChange={(e) => setRewardInput({ ...rewardInput, reward_text: e.target.value })}
                                    disabled={disabled}
                            ...
                            <div className="flex items-center space-x-2">
                                <input
                                    ...
                                    onChange={(e) => setRewardInput({ ...rewardInput, one_time: e.target.checked })}
                                    disabled={disabled}
                                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                                />
                                <Label htmlFor="one-time" className="text-sm font-medium text-gray-700">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => setRewardInput({ ...rewardInput, name: e.target.value })}
                                    disabled={disabled}
                            ...
                            <div>
                                <Label className="text-sm font-medium text-gray-700">Reward Description</Label>
                                <Textarea
                                    value={rewardInput.reward ?? ''}
                                    onChange={(e) => setRewardInput({ ...rewardInput, reward: e.target.value })}
                                    disabled={disabled}
